...
| `elasticsearch` | User provides an `http://` endpoint. Example: `http://host:9200` | User provides an `https://` endpoint and may set a custom CA via `CA_CERT`. Example: `https://host:9200` | Tabular | Elasticsearch Query DSL JSON. The index is specified in the request or connection properties. Example: `{"index":"products","query":{"match_all":{}}}` |
...
| `neo4j` | User provides a `neo4j://` URI. Example: `neo4j://host:7687` | User provides a `neo4j+s://` URI and may set a custom CA via `CA_CERT`. Example: `neo4j+s://host:7687` | Tabular | Cypher query. Example: `MATCH (n:Person) RETURN n.name LIMIT 10` |
...
| [HuggingFace](https://github.com/opendatahub-io/data-connect-hub/blob/main/config/connection-types/huggingface.yaml) | Built-in | `huggingface` | • `URI`<br>• `TOKEN` | — |
